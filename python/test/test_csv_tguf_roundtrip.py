...
def test_csv_tguf_roundtrip(resource_dir, output_tguf):
    ...
    builder.append_edges(tguf.Batch(
        ...
        msg=np.array([[float(r[c]) for c in msg_cols] for r in e_rows], dtype=np.float32),
        neg_dst=np.array([[int(r[c]) for c in neg_cols] for r in e_rows], dtype=np.int64) if neg_cols else None
    ))
    ...
    builder.append_labels(
        ...
        target=np.array([[float(r[c]) for c in label_cols] for r in l_rows], dtype=np.float32)
    )

    builder.append_node_feats(
        n_id=np.array([int(r['node_id']) for r in n_rows], dtype=np.int64),
        node_feat=np.array([[float(r[c]) for c in feat_cols] for r in n_rows], dtype=np.float32)
    )
    ...
    torch.testing.assert_close(label0.target[0], torch.tensor([1.0, 0.0], dtype=torch.float32))
    ...
    for i in range(len(n_ids)):
        if i % 5 == 0 and i <= 20:
            mult = i / 5.0
            expected = torch.tensor([1.0 * mult, 2.0 * mult, 3.0 * mult], dtype=torch.float32)
