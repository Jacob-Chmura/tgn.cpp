...
auto LastNeighborLoader::operator()(const torch::Tensor& global_n_id)
  ...
  assoc_.index_put_({unique_n_id},
                    torch::arange(unique_n_id.size(0), unique_n_id.options()));

  if (filtered_nbrs.numel() == 0) {
    TGUF_LOG_DEBUG("Sampler: No neighbors found for batch");
...
auto LastNeighborLoader::insert(const torch::Tensor& src,
                                const torch::Tensor& dst) -> void {
  ...
  auto dense_id = torch::arange(nodes.size(0), nodes.options()) % buffer_size_;
...
auto LastNeighborLoader::reset_state() -> void {
  TGUF_LOG_DEBUG("Sampler: Resetting state");
  cur_e_id_ = 0;
